  ...
  return (
    <div className="container mx-auto px-4 py-16">
      ...
      <div className="max-w-3xl mx-auto space-y-12">
        {content?.travelInfo && <section className="text-center"><div className="text-foreground/80 whitespace-pre-line">{content.travelInfo}</div></section>}
            ...
            <div className="grid gap-6">
              {content.hotels.map((hotel: any, i: number) => (
                <div key={i} className="border border-foreground/10 rounded-lg p-6">
                  ...
                  {hotel.url && <a href={hotel.url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">Book Now</a>}
                </div>
              ))}
            </div>
          </section>
        )}
        {!content && <div className="text-center text-foreground/60 py-8"><p>Travel information coming soon!</p><p className="text-sm mt-2">Check back later for accommodation details.</p></div>}
      </div>
    </div>
  );
}
